...
export function ResidentSignIn() {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const isResidentId = /^RES-\d{4}-\d{3}$/.test(formData.userId);
      
      if (isResidentId) {
        const q = query(collection(db, "residents"), where("residentId", "==", formData.userId));
      ...
      toast.error("Invalid Resident ID or password");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="w-full max-w-[380px]">
          ...
          {!showForgotPassword ? (
            <form onSubmit={(e) => e.preventDefault()}>
              <div className="mb-4">
                <label className="block text-xs text-gray-600 mb-1">Resident ID</label>
                <Input 
                  type="text" 
                  placeholder=""
                  value={formData.userId} 
                  onChange={(e) => setFormData({ ...formData, userId: e.target.value })} 
                  className="h-10 rounded-md border-2 border-gray-300 focus:border-green-600 focus:ring-2 focus:ring-green-600 focus:ring-offset-0 focus:outline-none transition-all duration-200"
                  required 
                />
                <p className="text-[10px] text-gray-400 mt-1">Use your Resident ID</p>
              </div>
              ...
              {resetSent ? (
                ...
                <form onSubmit={(e) => { e.preventDefault(); handleForgotPassword(); }}>
                  ...
                  <Input 
                    ...
                    className="h-10 rounded-md border-2 border-gray-300 focus:border-green-600 focus:ring-2 focus:ring-green-600 focus:ring-offset-0 focus:outline-none transition-all duration-200 mb-4"
                    required 
                  />
                  <Button 
                    type="submit" 
                    disabled={loading} 
                    className="w-full bg-green-700 hover:bg-green-800 h-10 rounded-md"
                  >
                    Send Reset Link
                  </Button>
                  <button 
                    onClick={() => setShowForgotPassword(false)} 
                    className="text-sm text-green-700 mt-4 block w-full text-center"
                  >
                    Back to Login
                  </button>
                </form>
